...
#include <stdio.h>
#include <stdarg.h>
static void OLED_IIC_Init(struct t_IIC_GPIO *iic);

struct IIC_Dev g_tOLEDDev = {
	.IIC_Init				= OLED_IIC_Init,
};

struct IIC_Dev* OLED_IIC_SoftwareReg(char *dev, GPIO_TypeDef *SDA_GPIOx, uint32_t SDA_Pin, GPIO_TypeDef *SCL_GPIOx, uint32_t SCL_Pin)
{
	g_tOLEDDev.IIC_GPIO.SDA_GPIOx 			= SDA_GPIOx;
	g_tOLEDDev.IIC_GPIO.SDA_GPIO_PINx 	= SDA_Pin;
	g_tOLEDDev.IIC_GPIO.SCL_GPIOx 			= SCL_GPIOx;
	g_tOLEDDev.IIC_GPIO.SCL_GPIO_PINx 	= SCL_Pin;
	g_tOLEDDev.IIC_GPIO.Addr						= OLED_ADDR;
	g_tOLEDDev.IIC_GPIO.mode						= IIC_SOFTWARE;
	strcpy(g_tOLEDDev.name, dev);
	RegisterIIC(&g_tOLEDDev);
	return SelectDefaultIIC(dev);
}

#ifdef HAL_I2C_MODULE_ENABLED
struct IIC_Dev* OLED_IIC_HardwareReg(char *dev, I2C_HandleTypeDef hi2cx)
{
	g_tOLEDDev.IIC_GPIO.Addr						= OLED_ADDR;
	g_tOLEDDev.IIC_GPIO.mode						= IIC_HARDWARE;
	g_tOLEDDev.IIC_GPIO.hi2cx						= hi2cx;
	strcpy(g_tOLEDDev.name, dev);
	RegisterIIC(&g_tOLEDDev);
	return SelectDefaultIIC(dev);
}
...
{
	IIC_Dev_WriteReg(&g_tOLEDDev.IIC_GPIO, 0x00, Command);
}
...
{
	IIC_Dev_Write_Regs(&g_tOLEDDev.IIC_GPIO, 0x40, Data, Count);
}
